...
func Importers(localDir, name string) ([]string, error) {
...
// recognizable is warned about and left untouched.
func Retarget(localDir, name, newPath string) ([]Change, error) {
	changes, _, err := retarget(localDir, name, newPath)
	return changes, err
}

func retarget(localDir, name, newPath string) ([]Change, []string, error) {
...
// Heal rewrites every broken import line in every host's entrypoint per
// implementation-plan.md §3's Heal table:
//   - path exists: keep.
//   - name resolves to exactly one unit: rewrite in every host referencing
//     it (via Retarget).
...
//
// activeFile may be the mirror symlink; hosts are compared by real path.
// Heal is idempotent.
func Heal(localDir, activeFile, modulesDir string, us []units.Unit, prune bool) ([]Change, []string, error) {
	activeReal, err := realPath(activeFile)
	if err != nil {
		return nil, nil, err
	}
	...
	// Track which names need retargeting, and to what, discovered while
	// scanning. Retarget itself loops every host, so collect names first
	// and apply once each to keep the result idempotent per bash's
	// behavior (imports::heal calls imports::retarget per broken line).
	...
	for _, file := range files {
		...
		fileReal, err := realPath(file)
		if err != nil {
			return nil, nil, err
		}
		isActive := fileReal == activeReal
