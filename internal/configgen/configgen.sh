...
# CONFIG_DIR/channels.toml, and discovers the shared kind folders (modules,
...
# symlink (see links::ensure_mirror), modules/system is the framework modules
# symlink (see links::ensure_framework_link), /local is the generated link
...
# The shared kind folders at CONFIG_DIR root — one directory per kind
...
configgen::generate() {
    ...
    if [[ -z "$main_user" ]]; then
        echo "Error: users array must have at least one user in $machine_toml" >&2
        exit 1
    fi
...
# each resolved against the shared kind folder at $CONFIG_DIR/<kind> — flat
# at the point of selection, so name -> path is ./<name>.nix when that file
# exists, else ./<name> (a directory carrying its own default.nix). The
# written default.nix is symlinked into the shared folder by
# links::ensure_mirror, so these relative import paths resolve against that
# folder, not machine_dir (a relative path inside a symlinked default.nix
# resolves against the symlink's own directory). No names means no imports.
#
# Used only for users — modules/services selection is each host's own
# hand-written entrypoint (.local/<host>/{modules,services}/default.nix).
configgen::generate_folder_default() {
    local machine_dir="$1" kind="$2"
    shift 2
    local names=("$@")
    local kind_dir="$CONFIG_DIR/$kind"
    ...
    for name in ${names[@]+"${names[@]}"}; do
        [[ -n "$name" ]] || continue
        if [[ -f "$kind_dir/$name.nix" ]]; then
            path="$name.nix"
        elif [[ -f "$kind_dir/$name/default.nix" ]]; then
            path="$name"
        else
            echo "Error: $kind '$name' not found under $kind_dir" >&2
            exit 1
        fi
        imports+=("    ./$path")
    done
