...
# symlink (see links::ensure_framework_pool), /local is the generated link
# to the active host's own .local/<host> (see links::ensure_local_link),
# and every .local/<host>'s own default.nix is generated by
...
# root) and local (the generated symlink into the active host's own
# .local/<host>, see links::ensure_local_link) — neither is itself a kind.
# The single source every derived list below is built from: adding a
# directory here is the only step needed to add a kind.
configgen::discover_kinds() {
    local entry name
    for entry in "$CONFIG_DIR"/*/; do
        [[ -d "$entry" ]] || continue
        name="$(basename "$entry")"
        [[ "$name" == "$(basename "$LOCAL_DIR")" ]] && continue
        [[ "$name" == "$CONFIGGEN_LOCAL_LINK_NAME" ]] && continue
        echo "$name"
    done | LC_ALL=C sort
}
